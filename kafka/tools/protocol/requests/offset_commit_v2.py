...
class OffsetCommitV2Request(BaseRequest):
    ...
    def process_arguments(self, cmd_args):
        ...
        return [cmd_args[0], int(cmd_args[1]), cmd_args[2], cmd_args[3], topics]

    def response(self, correlation_id):
        return OffsetCommitV2Response(correlation_id)

    @classmethod
    def show_help(cls):
        print("Request:     {0}V{1}".format(cls.cmd, cls.api_version))
        print("Format:      {0}V{1} group_id group_generation_id member_id retention_time (topic (partition,offset[,metadata] ...) ...)".format(
