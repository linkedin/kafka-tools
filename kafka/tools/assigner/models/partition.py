...
class Partition:
  ...
  def dict_for_move(self):
    return {"topic": self.topic.name, "partition": self.num, "replicas": self.replicas}
  ...
  def remove_replica(self, broker):
    try:
      position = self.replicas.index(broker.id)
    except ValueError:
      raise ReplicaNotFoundException

    broker.partitions[position].remove(self)
    self.replicas.remove(broker.id)
  ...
  def swap_replicas(self, remove_broker, add_broker):
    try:
      position = self.replicas.index(remove_broker.id)
    except ValueError:
      raise ReplicaNotFoundException
  ...
  def swap_replica_positions(self, broker1, broker2):
    try:
      p1 = self.replicas.index(broker1.id)
      p2 = self.replicas.index(broker2.id)
    except ValueError:
      raise ReplicaNotFoundException
    ...
    self._add_broker_partition(p2, broker2)
  ...
  def _remove_broker_partition(self, broker):
    pos = self.replicas.index(broker.id)
