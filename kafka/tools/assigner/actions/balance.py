...
class ActionBalance(ActionModule):
  ...
  def __init__(self, args, cluster):
    ...
    self.balance_types = {cls.name: cls for cls in get_modules(kafka.tools.assigner.actions.balancemodules, kafka.tools.assigner.actions.balancemodules.ActionBalanceModule)}

    # Initialize all the modules to use
    self.modules = []
    for bmodule in args.types:
      self.modules.append(balance_types[bmodule](args, cluster))

  @classmethod
  def _add_args(cls, parser):
    # We'll need to build this list here as well as in the constructor
    balance_actions = get_modules(kafka.tools.assigner.actions.balancemodules, kafka.tools.assigner.actions.balancemodules.ActionBalanceModule)
    parser.add_argument('-t', '--types', help="Balance types to perform. Multiple may be specified and they will be run in order", required=True,
                        choices=[cls.name for cls in balance_actions], nargs='*')
