...
class ActionClone(ActionModule):
  ...
  def process_cluster(self):
    source_set = set(self.args.sources)
    for partition in self.cluster.partitions():
      if len(source_set & set(partition.replicas)) > 0:
        if self.to_broker in partition.replicas:
          log.warn("Target broker (ID {0}) is already in the replica list for {1}:{2}".format(self.to_broker, partition.topic.name, partition.num))

          # If the broker is already in the replica list, it ALWAYS becomes the leader
          if self.to_broker != partition.replicas[0]:
            partition.swap_replica_positions(self.cluster.brokers[self.to_broker], self.cluster.brokers[partition.replicas[0]])
        else:
          ...
          if partition.replicas[0] in self.brokers:
            partition.add_replica(self.cluster.brokers[self.to_broker], 0)
          else:
            partition.add_replica(self.cluster.brokers[self.to_broker], 1)
